...
console.log(`   ✓ Expected: 4 areas (main-stage, dance-workshops, music-workshops, salsa-talks)`);
...
TIMELINE_CONFIG.forEach((areaConfig) => {
  ...
  areaConfig.schedules.forEach((schedule) => {
    console.log(`       • ${schedule.dayWeekday}: ${schedule.timeline.length} events`);
  });
});
...
const saturdayDanceWorkshops = getTimelineForAreaAndDay('dance-workshops', 'saturday');
...
console.log(`   ✓ Invalid area returns: ${invalidArea.length === 0 ? 'empty array (correct)' : 'ERROR'}`);
console.log(`   ✓ Invalid day returns: ${invalidDay.length === 0 ? 'empty array (correct)' : 'ERROR'}`);

// Test 3: Backward compatibility
console.log('\n3️⃣  Testing backward compatibility exports...');
console.log(`   ✓ mainStageSaturdayTimeline: ${mainStageSaturdayTimeline.length} events`);
console.log(`   ✓ mainStageSundayTimeline: ${mainStageSundayTimeline.length} events`);

const compatMatches = 
  mainStageSaturdayTimeline.length === saturdayMainStage.length &&
  mainStageSundayTimeline.length === sundayMainStage.length;
console.log(`   ✓ Old exports match new helper: ${compatMatches ? 'YES' : 'NO (ERROR!)'}`);
...
console.log(`   ✓ Areas with events on Monday: ${mondayAreas.length} (should be 0)`);
...
TIMELINE_CONFIG.forEach((areaConfig) => {
  // Check area is valid
  const validAreas = ['main-stage', 'dance-workshops', 'music-workshops', 'salsa-talks'];
...
console.log(`   ✓ All schedules valid: ${allSchedulesValid ? 'YES' : 'NO (ERROR!)'}`);
...
console.log(`   ✓ All event IDs follow format: ${allEventIdsValid ? 'YES' : 'NO (ERROR!)'}`);
...
console.log(`   ✓ Case-insensitive: ${satLower.length === satUpper.length && satUpper.length === satMixed.length ? 'YES' : 'NO (ERROR!)'}`);
